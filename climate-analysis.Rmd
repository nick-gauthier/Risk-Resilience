...
saveRDS(trace.dat, 'trace_data_points.rds')
```

```{r}
trace.dat <- readRDS('trace_data_points.rds')
```

## Trend Analysis

Let's use the **EMD** package to calculate trend lines using the empirical mode decomposition approach.

```{r, echo=FALSE, fig.cap="Empirical mode decomposition example", fig.height = 10, message=F, cache = F}
...
Now organize the temperature and precipitation data to make plotting easier using functions from **tidyr**, along with **EMD** for the trends.
```{r cache = F}
...
trace.reg.avg <- rbind(
  ...
  mutate(Year = as.numeric(substring(Year, 3))) %>%
  filter(Year > 6) 
saveRDS(trace.reg.avg, 'trace_data_regional.rds')
```
```{r}
trace.reg.avg <- readRDS('trace_data_regional.rds')
```
...
```{r eval = F}
# Preprocessing of raw data, skip if using included .RDS files
...
saveRDS(tmean.obs, 'tmean_obs.rds')
saveRDS(p.obs, 'prec_obs.rds')
```

```{r}
tmean.obs <- readRDS('tmean_obs.rds')
p.obs <- readRDS('prec_obs.rds')
```
...
saveRDS(tcw, 'tcw_reanalysis.rds')
saveRDS(msl, 'msl_reanalysis.rds')
saveRDS(t2m, 't2m_reanalysis.rds')
saveRDS(lsp, 'lsp_reanalysis.rds')
saveRDS(cp, 'cp_reanalysis.rds')
```

```{r}
tcw <- readRDS('tcw_reanalysis.rds')
msl <- readRDS('msl_reanalysis.rds')
t2m <- readRDS('t2m_reanalysis.rds')
lsp <- readRDS('lsp_reanalysis.rds')
cp <- readRDS('cp_reanalysis.rds')
```
...
saveRDS(t2m, 't2m_mh.rds')
saveRDS(msl, 'msl_mh.rds')
saveRDS(cp, 'cp_mh.rds')
saveRDS(lsp, 'lsp_mh.rds')
saveRDS(tcw, 'tcw_mh.rds')
```

```{r}
t2m <- readRDS('t2m_mh.rds')
msl <- readRDS('msl_mh.rds')
cp <- readRDS('cp_mh.rds')
lsp <- readRDS('lsp_mh.rds')
tcw <- readRDS('tcw_mh.rds')
```
...
saveRDS(t2m, 't2m_lgm.rds')
saveRDS(msl, 'msl_lgm.rds')
saveRDS(cp, 'cp_lgm.rds')
saveRDS(lsp, 'lsp_lgm.rds')
saveRDS(tcw, 'tcw_lgm.rds')
```

```{r}
t2m <- readRDS('t2m_lgm.rds')
msl <- readRDS('msl_lgm.rds')
cp <- readRDS('cp_lgm.rds')
lsp <- readRDS('lsp_lgm.rds')
tcw <- readRDS('tcw_lgm.rds')
```
...
# capped at -60 to reveal patterns
