...
samp.pts <- matrix(c(0,40, 4,44, 12, 46, 12, 43), ncol = 2, byrow = T) %>% as_data_frame
...
```{r samppoints}
samp.pts <- matrix(c(0, 40, 4, 44, 12, 46, 12, 43), 
                   ncol = 2, byrow = T)
...
ggplot(emd.dat, aes(x = `prc,Southwest`, y =`tmp,Southwest`, color = Year)) +
  geom_path(linejoin = 'round') + 
  scale_color_gradientn(colors = myPalette(100))
...
library(maptools)
trace.poly <- readShapePoly('t31 grid/t31grid.shp')

over(SpatialPoints(samp.pts), trace.poly)
trace.smp <- trace.poly[c(1296,1201,1107),]


equilibrium.vals <- brick(c(mean(lgm.tmp),
                            mean(mh.tmp),
                            sum(lgm.prc),
                            sum(mh.prc))) %>%
  raster::extract(trace.smp, fun = mean, na.rm = T) %>%
  tbl_df %>%
  transmute(Region = c('Southwest', 'North Central', 'Northeast'),
         lgm.tmp = layer.1,
         mh.tmp = layer.2,
         lgm.prc = layer.3,
         mh.prc = layer.4) %>%
  gather(key, value, - Region) %>%
  separate(key, c('Year', 'Variable')) %>%
  mutate(Variable = ifelse(Variable == 'tmp', 'Temperature (°C)', 'Precipitation (mm)'),
         Year = ifelse(Year == 'lgm', 22, 6))
...
ggplot(data = trace.plot, aes(x = Year, y = value)) +
  ...
  geom_point(aes(color = Variable), alpha = .3) +
  geom_line(data = trace.emd, size = 1.2, color = "black", alpha = .8) +
  geom_point(data = equilibrium.vals) +
  scale_x_reverse(breaks = seq(6,22,4)) +
  labs(x = '1,000 Years BP', y = '') +
  guides(color = "none") +
  theme_bw(base_size = 20) +
